...
class ExpensesAccountController extends Controller
{
  ...
  function __construct(){
    $this->form_name ='expenses-accounts';
  }
  public function index(Request $request)
  {
    $response = [];
    if (!Gate::allows('expenses-account-read') && !Auth::user()->administrator){
    ...
    $records = ExpensesAccount::where($funnel["filters"]);
    $params = [];
  ...
  {
    $response = [];
    if (!Gate::allows('expenses-account-create') && !Auth::user()->administrator){
      $response['status'] = 'error';
      $response['msg'] = 'No estas autorizado';
      return response()->json($response, 403);
    }
    #inicialice objet ExpensesAccount
    $objExpensesAccount=new ExpensesAccount();
    #Validations
    ...
    #funcion que esta en el método expenses-accounts y se encarga de
    #generar un arreglo compatible con validate, con los campos disponibles en el setting del form el el arreglo validations
    $array_validator=$objExpensesAccount->validationsReturns($arrayFields,'store',false);
    ...
    DB::transaction(function () use ($params) {
      $id = DB::table('expenses_accounts')->insertGetId($params['record']);
      $record = [];
      $record['record_id'] = $id;
      $record['form_code'] = 'F09';
      $record['form_title'] = 'Alta de cuenta de egreso';
      $record['name'] = 'expenses-accounts';
      $record['period'] = null;
  ...
  public function show(ExpensesAccount $expensesAccount)
  {
    $userId = Auth::id();
    $record = DB::table('records')->select('id')->where([
      ['name', '=', 'expenses-accounts'],
      ['record_id', '=', $expensesAccount['id']]
    ])->first();
    ...
    $response = [];
    if (!Gate::allows('expenses-account-read') && !Auth::user()->administrator && $count == 0){
      $response['status'] = 'error';
      $response['msg'] = 'No estas autorizado';
      return response()->json($response, 403);
    }
    $record = ExpensesAccount::find($expensesAccount['id']);
    $response['data'] = $record;
    return response()->json($response, 200);
  }
 
  public function update(Request $request, ExpensesAccount $expensesAccount)
  {
    $userId = Auth::id();
    $record = DB::table('records')->select('id')->where([
      ['name', '=', 'expenses-accounts'],
    ...
    $response = [];
    if (!Gate::allows('expenses-account-edit') && !Auth::user()->administrator && $count == 0){
      $response['status'] = 'error';
      $response['msg'] = 'No estas autorizado';
      return response()->json($response, 403);
    }
    $record = DB::table('expenses_accounts')->select('blocked')->where('id', $expensesAccount['id'])->first();
    if($record->blocked === 1) {
      $response['status'] = 'error';
   ...
   #funcion que esta en el método expenses-accounts y se encarga de
   #generar un arreglo compatible con validate, con los campos disponibles en el setting del form el el arreglo validations
   $array_validator=$objExpensesAccount->validationsReturns($arrayFields,'update',$expensesAccount['id']);
   $validated_data= Validator::make($request->all(),$array_validator);
   if($validated_data->fails()){
     #si hay algun elemento que no es valido, retorna un arreglo de errores
     $response['status'] = 'error';
   ...
   #fin validaciones
    ...
    $params['t3'] = '';
    DB::transaction(function () use ($params) {
      DB::table('expenses_accounts')->where('id', $params['record']['id'])->update($params['record']);
      $record = [];
      $record['form_code'] = 'F09';
      $record['form_title'] = 'Alta de cuenta de egreso';
      $record['name'] = 'expenses-accounts';
      ...
      $record['updated_by'] = $params['user_id'];
      $record["updated_at"] = Carbon::now()->toDateTimeString();
      DB::table('records')->where([['record_id', '=', $params['record']['id']],['name', '=', 'expenses-accounts']])->update($record);
    });
    $response['success'] = true;
    return response()->json($response, 200);
  }
  public function destroy(ExpensesAccount $expensesAccount)
  {
    $userId = Auth::id();
    if (!Gate::allows('expenses-account-delete') && !Auth::user()->administrator){
      $response['status'] = 'error';
      $response['msg'] = 'No estas autorizado';
      return response()->json($response, 403);
    }
    ...
    $params = [];
    $params['record'] = $expensesAccount;
    DB::transaction(function () use ($params) {
      ExpensesAccount::destroy($params['record']['id']);
  ...
  public function all(Request $request)
  {
     ...
     $records = ExpensesAccount::where($req)->get();
     $response['data'] = $records;
     return response()->json($response, 200);
  }
  public function search(Request $request) {
    ...
    $records = ExpensesAccount::where($filter)
  ...
  public function do(Request $request)
  {
    ...
    $params['req'] = $req;
    DB::transaction(function () use ($params) {
      DB::table('expenses_accounts')->where('id', $params['req']['id'])->update(['done' => $params['req']['value']]);
      DB::table('records')->where([['record_id', '=', $params['req']['id']],['name', '=', 'expenses-accounts']])->update(['done' => $params['req']['value']]);
    });
    $response = [];
    $response['req'] = $req;
    $response['success'] = true;
  ...
  public function cancel(Request $request)
  {
    ...
    $params['req'] = $req;
    DB::transaction(function () use ($params) {
      DB::table('expenses_accounts')->where('id', $params['req']['id'])->update(['cancelled' => $params['req']['value']]);
      DB::table('records')->where([['record_id', '=', $params['req']['id']],['name', '=', 'expenses-accounts']])->update(['cancelled' => $params['req']['value']]);
    });
    $response = [];
    $response['req'] = $req;
    $response['success'] = true;
  ...
  public function block(Request $request)
  {
    ...
    $params['req'] = $req;
    DB::transaction(function () use ($params) {
      DB::table('expenses_accounts')->where('id', $params['req']['id'])->update(['blocked' => $params['req']['value']]);
      DB::table('records')->where([['record_id', '=', $params['req']['id']],['name', '=', 'expenses-accounts']])->update(['blocked' => $params['req']['value']]);
    });
    $response = [];
    $response['req'] = $req;
    $response['success'] = true;
  ...
  public function downloadPdf($id)
  {
    ...
    $record = ExpensesAccount::find($id);
    $response['data'] = $record;
