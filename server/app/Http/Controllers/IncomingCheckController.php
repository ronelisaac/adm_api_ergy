...
class IncomingCheckController extends Controller
{
  #variable que almacena el nombre del formulario
  private $form_name;
  #Constructor
  function __construct(){
    $this->form_name ='incoming-checks';
  }
  public function index(Request $request)
  {
    $response = [];
    if (!Gate::allows('incoming-check-read') && !Auth::user()->administrator){
    ...
    $records = IncomingCheck::where($funnel["filters"]);
    $params = [];
    ...
    ->map(function ($record) {
      ...
      $bank = [];
      $bankString = [];
      $bankData = IncomingCheck::find($record->id);
      ...
      $user = [];
      $userString = [];
      $userData = IncomingCheck::find($record->id);
  ...
  {
    $response = [];
    if (!Gate::allows('incoming-check-create') && !Auth::user()->administrator){
      $response['status'] = 'error';
      $response['msg'] = 'No estas autorizado';
      return response()->json($response, 403);
    }
    #inicialice objet IncomingCheck
    $objIncomingCheck=new IncomingCheck();
    #Validations
    ...
    #funcion que esta en el método incoming-checks y se encarga de
    #generar un arreglo compatible con validate, con los campos disponibles en el setting del form el el arreglo validations
    $array_validator=$objIncomingCheck->validationsReturns($arrayFields,'store',false);
    ...
    DB::transaction(function () use ($params) {
      $id = DB::table('incoming_checks')->insertGetId($params['record']);
      $record = [];
      $record['record_id'] = $id;
      $record['form_code'] = 'F011';
      $record['form_title'] = 'Alta de cheque recibido';
      $record['name'] = 'incoming-checks';
      $record['period'] = null;
  ...
  public function show(IncomingCheck $incomingCheck)
  {
    $userId = Auth::id();
    $record = DB::table('records')->select('id')->where([
      ['name', '=', 'incoming-checks'],
      ['record_id', '=', $incomingCheck['id']]
    ])->first();
    ...
    $response = [];
    if (!Gate::allows('incoming-check-read') && !Auth::user()->administrator && $count == 0){
      $response['status'] = 'error';
      $response['msg'] = 'No estas autorizado';
      return response()->json($response, 403);
    }
    $record = IncomingCheck::find($incomingCheck['id']);
    $bank = $record->bank;
    $banks = [];
  ...
  public function update(Request $request, IncomingCheck $incomingCheck)
  {
    $userId = Auth::id();
    $record = DB::table('records')->select('id')->where([
      ['name', '=', 'incoming-checks'],
    ...
    $response = [];
    if (!Gate::allows('incoming-check-edit') && !Auth::user()->administrator && $count == 0){
      $response['status'] = 'error';
      $response['msg'] = 'No estas autorizado';
      return response()->json($response, 403);
    }
    $record = DB::table('incoming_checks')->select('blocked')->where('id', $incomingCheck['id'])->first();
    if($record->blocked === 1) {
      $response['status'] = 'error';
   ...
   #funcion que esta en el método incoming-checks y se encarga de
   #generar un arreglo compatible con validate, con los campos disponibles en el setting del form el el arreglo validations
   $array_validator=$objIncomingCheck->validationsReturns($arrayFields,'update',$incomingCheck['id']);
   $validated_data= Validator::make($request->all(),$array_validator);
   if($validated_data->fails()){
     #si hay algun elemento que no es valido, retorna un arreglo de errores
     $response['status'] = 'error';
   ...
   #fin validaciones
    ...
    $params['t3'] = (count($request->input('banks')['body']) > 0 ? $request->input('banks')['body'][0]['name'] : "");
    DB::transaction(function () use ($params) {
      DB::table('incoming_checks')->where('id', $params['record']['id'])->update($params['record']);
      $record = [];
      $record['form_code'] = 'F011';
      $record['form_title'] = 'Alta de cheque recibido';
      $record['name'] = 'incoming-checks';
      ...
      $record['updated_by'] = $params['user_id'];
      $record["updated_at"] = Carbon::now()->toDateTimeString();
      DB::table('records')->where([['record_id', '=', $params['record']['id']],['name', '=', 'incoming-checks']])->update($record);
    });
    $response['success'] = true;
    return response()->json($response, 200);
  }
  public function destroy(IncomingCheck $incomingCheck)
  {
    $userId = Auth::id();
    if (!Gate::allows('incoming-check-delete') && !Auth::user()->administrator){
      $response['status'] = 'error';
      $response['msg'] = 'No estas autorizado';
      return response()->json($response, 403);
    }
    ...
    $params = [];
    $params['record'] = $incomingCheck;
    DB::transaction(function () use ($params) {
      IncomingCheck::destroy($params['record']['id']);
  ...
  public function all(Request $request)
  {
     ...
     $records = IncomingCheck::where($req)->get();
     $response['data'] = $records;
     return response()->json($response, 200);
  }
  public function search(Request $request) {
    ...
    $records = IncomingCheck::where($filter)
  ...
  public function do(Request $request)
  {
    ...
    $params['req'] = $req;
    DB::transaction(function () use ($params) {
      DB::table('incoming_checks')->where('id', $params['req']['id'])->update(['done' => $params['req']['value']]);
      DB::table('records')->where([['record_id', '=', $params['req']['id']],['name', '=', 'incoming-checks']])->update(['done' => $params['req']['value']]);
    });
    $response = [];
    $response['req'] = $req;
    $response['success'] = true;
  ...
  public function cancel(Request $request)
  {
    ...
    $params['req'] = $req;
    DB::transaction(function () use ($params) {
      DB::table('incoming_checks')->where('id', $params['req']['id'])->update(['cancelled' => $params['req']['value']]);
      DB::table('records')->where([['record_id', '=', $params['req']['id']],['name', '=', 'incoming-checks']])->update(['cancelled' => $params['req']['value']]);
    });
    $response = [];
    $response['req'] = $req;
    $response['success'] = true;
  ...
  public function block(Request $request)
  {
    ...
    $params['req'] = $req;
    DB::transaction(function () use ($params) {
      DB::table('incoming_checks')->where('id', $params['req']['id'])->update(['blocked' => $params['req']['value']]);
      DB::table('records')->where([['record_id', '=', $params['req']['id']],['name', '=', 'incoming-checks']])->update(['blocked' => $params['req']['value']]);
    });
    $response = [];
    $response['req'] = $req;
    $response['success'] = true;
