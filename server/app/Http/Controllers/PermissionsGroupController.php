...
class PermissionsGroupController extends Controller
{
  ...
  function __construct(){
    $this->form_name ='permissions-groups';
  }
  public function index(Request $request)
  {
    $response = [];
    if (!Gate::allows('permissions-group-read') && !Auth::user()->administrator){
    ...
    $records = PermissionsGroup::where($funnel["filters"]);
    $params = [];
  ...
  {
    $response = [];
    if (!Gate::allows('permissions-group-create') && !Auth::user()->administrator){
      $response['status'] = 'error';
      $response['msg'] = 'No estas autorizado';
      return response()->json($response, 403);
    }
    #inicialice objet PermissionsGroup
    ...
    #funcion que esta en el método permissions-groups y se encarga de
    #generar un arreglo compatible con validate, con los campos disponibles en el setting del form el el arreglo validations
    $array_validator=$objPermissionsGroup->validationsReturns($arrayFields,'store',false);
    ...
    DB::transaction(function () use ($params) {
      $id = DB::table('permissions_groups')->insertGetId($params['record']);
      $record = [];
      $record['record_id'] = $id;
      $record['form_code'] = 'F04';
      $record['form_title'] = 'Alta de grupo de permisos';
      $record['name'] = 'permissions-groups';
      $record['period'] = null;
  ...
  public function show(PermissionsGroup $permissionsGroup)
  {
    ...
    $response = [];
    if (!Gate::allows('permissions-group-read') && !Auth::user()->administrator && $count == 0){
      $response['status'] = 'error';
      $response['msg'] = 'No estas autorizado';
      return response()->json($response, 403);
    }
    $record = PermissionsGroup::find($permissionsGroup['id']);
  ...
  public function update(Request $request, PermissionsGroup $permissionsGroup)
  {
    ...
    $response = [];
    if (!Gate::allows('permissions-group-edit') && !Auth::user()->administrator && $count == 0){
      $response['status'] = 'error';
      $response['msg'] = 'No estas autorizado';
      return response()->json($response, 403);
    }
    $record = DB::table('permissions_groups')->select('blocked')->where('id', $permissionsGroup['id'])->first();
    if($record->blocked === 1) {
      $response['status'] = 'error';
   ...
   #funcion que esta en el método permissions-groups y se encarga de
   #generar un arreglo compatible con validate, con los campos disponibles en el setting del form el el arreglo validations
   $array_validator=$objPermissionsGroup->validationsReturns($arrayFields,'update',$permissionsGroup['id']);
   $validated_data= Validator::make($request->all(),$array_validator);
   if($validated_data->fails()){
     #si hay algun elemento que no es valido, retorna un arreglo de errores
     $response['status'] = 'error';
   ...
   #fin validaciones
    ...
    $params['t3'] = '';
    DB::transaction(function () use ($params) {
      DB::table('permissions_groups')->where('id', $params['record']['id'])->update($params['record']);
      $record = [];
      $record['form_code'] = 'F04';
      $record['form_title'] = 'Alta de grupo de permisos';
      $record['name'] = 'permissions-groups';
      ...
      $record['updated_by'] = $params['user_id'];
      $record["updated_at"] = Carbon::now()->toDateTimeString();
      DB::table('records')->where([['record_id', '=', $params['record']['id']],['name', '=', 'permissions-groups']])->update($record);
    });
    $response['success'] = true;
    return response()->json($response, 200);
  }
  ...
  {
    $userId = Auth::id();
    if (!Gate::allows('permissions-group-delete') && !Auth::user()->administrator){
      $response['status'] = 'error';
      $response['msg'] = 'No estas autorizado';
      return response()->json($response, 403);
    }
    ...
    $params = [];
    $params['record'] = $permissionsGroup;
    DB::transaction(function () use ($params) {
      PermissionsGroup::destroy($params['record']['id']);
  ...
  public function all(Request $request)
  {
     ...
     $records = PermissionsGroup::where($req)->get();
     $response['data'] = $records;
     return response()->json($response, 200);
  }
  public function search(Request $request) {
    ...
    $records = PermissionsGroup::where($filter)
  ...
  public function do(Request $request)
  {
    ...
    $params['req'] = $req;
    DB::transaction(function () use ($params) {
      DB::table('permissions_groups')->where('id', $params['req']['id'])->update(['done' => $params['req']['value']]);
      DB::table('records')->where([['record_id', '=', $params['req']['id']],['name', '=', 'permissions-groups']])->update(['done' => $params['req']['value']]);
    });
    $response = [];
    $response['req'] = $req;
    $response['success'] = true;
  ...
  public function cancel(Request $request)
  {
    ...
    $params['req'] = $req;
    DB::transaction(function () use ($params) {
      DB::table('permissions_groups')->where('id', $params['req']['id'])->update(['cancelled' => $params['req']['value']]);
      DB::table('records')->where([['record_id', '=', $params['req']['id']],['name', '=', 'permissions-groups']])->update(['cancelled' => $params['req']['value']]);
    });
    $response = [];
    $response['req'] = $req;
    $response['success'] = true;
  ...
  public function block(Request $request)
  {
    ...
    $params['req'] = $req;
    DB::transaction(function () use ($params) {
      DB::table('permissions_groups')->where('id', $params['req']['id'])->update(['blocked' => $params['req']['value']]);
      DB::table('records')->where([['record_id', '=', $params['req']['id']],['name', '=', 'permissions-groups']])->update(['blocked' => $params['req']['value']]);
    });
    $response = [];
    $response['req'] = $req;
    $response['success'] = true;
  ...
  public function downloadPdf($id)
  {
    ...
    $record = PermissionsGroup::find($id);
    $response['data'] = $record;
