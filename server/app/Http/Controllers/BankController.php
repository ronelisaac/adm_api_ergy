...
class BankController extends Controller
{
  ...
  function __construct(){
    $this->form_name ='banks';
  }
  public function index(Request $request)
  {
    $response = [];
    if (!Gate::allows('bank-read') && !Auth::user()->administrator){
    ...
    $records = Bank::where($funnel["filters"]);
    $params = [];
  ...
  {
    $response = [];
    if (!Gate::allows('bank-create') && !Auth::user()->administrator){
      $response['status'] = 'error';
      $response['msg'] = 'No estas autorizado';
      return response()->json($response, 403);
    }
    #inicialice objet Bank
    $objBank=new Bank();
    #Validations
    ...
    DB::transaction(function () use ($params) {
      $id = DB::table('banks')->insertGetId($params['record']);
      $record = [];
      $record['record_id'] = $id;
      $record['form_code'] = 'F11';
      $record['form_title'] = 'Alta de bancos';
      $record['name'] = 'banks';
      $record['period'] = null;
  ...
  public function show(Bank $bank)
  {
    ...
    $response = [];
    if (!Gate::allows('bank-read') && !Auth::user()->administrator && $count == 0){
      $response['status'] = 'error';
      $response['msg'] = 'No estas autorizado';
      return response()->json($response, 403);
    }
    $record = Bank::find($bank['id']);
  ...
  public function update(Request $request, Bank $bank)
  {
    ...
    $response = [];
    if (!Gate::allows('bank-edit') && !Auth::user()->administrator && $count == 0){
      $response['status'] = 'error';
      $response['msg'] = 'No estas autorizado';
      return response()->json($response, 403);
    }
    $record = DB::table('banks')->select('blocked')->where('id', $bank['id'])->first();
    if($record->blocked === 1) {
      $response['status'] = 'error';
   ...
   #funcion que esta en el método banks y se encarga de
   #generar un arreglo compatible con validate, con los campos disponibles en el setting del form el el arreglo validations
   $array_validator=$objBank->validationsReturns($arrayFields,'update',$bank['id']);
   $validated_data= Validator::make($request->all(),$array_validator);
   if($validated_data->fails()){
     #si hay algun elemento que no es valido, retorna un arreglo de errores
     $response['status'] = 'error';
   ...
   #fin validaciones
    ...
    $params['t3'] = '';
    DB::transaction(function () use ($params) {
      DB::table('banks')->where('id', $params['record']['id'])->update($params['record']);
      $record = [];
      $record['form_code'] = 'F11';
      $record['form_title'] = 'Alta de bancos';
      $record['name'] = 'banks';
      ...
      $record['updated_by'] = $params['user_id'];
      $record["updated_at"] = Carbon::now()->toDateTimeString();
      DB::table('records')->where([['record_id', '=', $params['record']['id']],['name', '=', 'banks']])->update($record);
    });
    $response['success'] = true;
    return response()->json($response, 200);
  }
  ...
  {
    $userId = Auth::id();
    if (!Gate::allows('bank-delete') && !Auth::user()->administrator){
      $response['status'] = 'error';
      $response['msg'] = 'No estas autorizado';
      return response()->json($response, 403);
    }
  ...
  public function do(Request $request)
  {
    ...
    $params['req'] = $req;
    DB::transaction(function () use ($params) {
      DB::table('banks')->where('id', $params['req']['id'])->update(['done' => $params['req']['value']]);
      DB::table('records')->where([['record_id', '=', $params['req']['id']],['name', '=', 'banks']])->update(['done' => $params['req']['value']]);
    });
    $response = [];
    $response['req'] = $req;
    $response['success'] = true;
  ...
  public function cancel(Request $request)
  {
    ...
    $params['req'] = $req;
    DB::transaction(function () use ($params) {
      DB::table('banks')->where('id', $params['req']['id'])->update(['cancelled' => $params['req']['value']]);
      DB::table('records')->where([['record_id', '=', $params['req']['id']],['name', '=', 'banks']])->update(['cancelled' => $params['req']['value']]);
    });
    $response = [];
    $response['req'] = $req;
    $response['success'] = true;
  ...
  public function block(Request $request)
  {
    ...
    $params['req'] = $req;
    DB::transaction(function () use ($params) {
      DB::table('banks')->where('id', $params['req']['id'])->update(['blocked' => $params['req']['value']]);
      DB::table('records')->where([['record_id', '=', $params['req']['id']],['name', '=', 'banks']])->update(['blocked' => $params['req']['value']]);
    });
    $response = [];
    $response['req'] = $req;
    $response['success'] = true;
