...
class AddressController extends Controller
{
  ...
  function __construct(){
    $this->form_name ='addresses';
  }
  public function index(Request $request)
  {
    $response = [];
    if (!Gate::allows('address-read') && !Auth::user()->administrator){
  ...
  {
    $response = [];
    if (!Gate::allows('address-create') && !Auth::user()->administrator){
      $response['status'] = 'error';
      $response['msg'] = 'No estas autorizado';
      return response()->json($response, 403);
    }
    #inicialice objet Address
    ...
    #funcion que esta en el método addresses y se encarga de
    #generar un arreglo compatible con validate, con los campos disponibles en el setting del form el el arreglo validations
    $array_validator=$objAddress->validationsReturns($arrayFields,'store',false);
    ...
    $record = [];
    $params = [];
    $record['address_line'] = $request->input('address_line');
    $record['long'] = $request->input('long');
    $record['lat'] = $request->input('lat');
    $record['street'] = $request->input('street');
    $record['street_number'] = $request->input('street_number');
    $record['neighborhood'] = $request->input('neighborhood');
    $record['locality'] = $request->input('locality');
    $record['district'] = $request->input('district');
    $record['country'] = $request->input('country');
    $record['postal_code'] = $request->input('postal_code');
    ...
    $params['t1'] = (null !== $request->input('address_line') ? $request->input('address_line') : "");
    $params['t2'] = '';
    $params['t3'] = '';
    DB::transaction(function () use ($params) {
      $id = DB::table('addresses')->insertGetId($params['record']);
      $record = [];
      $record['record_id'] = $id;
      $record['form_code'] = 'F04';
      $record['form_title'] = 'Alta de domicilio';
      $record['name'] = 'addresses';
      $record['period'] = null;
  ...
  public function show(Address $address)
  {
    ...
    $response = [];
    if (!Gate::allows('address-read') && !Auth::user()->administrator && $count == 0){
      $response['status'] = 'error';
      $response['msg'] = 'No estas autorizado';
      return response()->json($response, 403);
    }
    $record = Address::find($address['id']);
  ...
  public function update(Request $request, Address $address)
  {
    ...
    $response = [];
    if (!Gate::allows('address-edit') && !Auth::user()->administrator && $count == 0){
      $response['status'] = 'error';
      $response['msg'] = 'No estas autorizado';
      return response()->json($response, 403);
    }
    $record = DB::table('addresses')->select('blocked')->where('id', $address['id'])->first();
    if($record->blocked === 1) {
      $response['status'] = 'error';
   ...
   #funcion que esta en el método addresses y se encarga de
   #generar un arreglo compatible con validate, con los campos disponibles en el setting del form el el arreglo validations
   $array_validator=$objAddress->validationsReturns($arrayFields,'update',$address['id']);
   $validated_data= Validator::make($request->all(),$array_validator);
   if($validated_data->fails()){
     #si hay algun elemento que no es valido, retorna un arreglo de errores
     $response['status'] = 'error';
   ...
   #fin validaciones
    ...
    $params['t3'] = '';
    DB::transaction(function () use ($params) {
      DB::table('addresses')->where('id', $params['record']['id'])->update($params['record']);
      $record = [];
      $record['form_code'] = 'F04';
      $record['form_title'] = 'Alta de domicilio';
      $record['name'] = 'addresses';
      ...
      $record['updated_by'] = $params['user_id'];
      $record["updated_at"] = Carbon::now()->toDateTimeString();
      DB::table('records')->where([['record_id', '=', $params['record']['id']],['name', '=', 'addresses']])->update($record);
    });
    $response['success'] = true;
    return response()->json($response, 200);
  }
  ...
  {
    $userId = Auth::id();
    if (!Gate::allows('address-delete') && !Auth::user()->administrator){
      $response['status'] = 'error';
      $response['msg'] = 'No estas autorizado';
      return response()->json($response, 403);
    }
    ...
    DB::transaction(function () use ($params) {
      Address::destroy($params['record']['id']);
  ...
  public function do(Request $request)
  {
    ...
    $params['req'] = $req;
    DB::transaction(function () use ($params) {
      DB::table('addresses')->where('id', $params['req']['id'])->update(['done' => $params['req']['value']]);
      DB::table('records')->where([['record_id', '=', $params['req']['id']],['name', '=', 'addresses']])->update(['done' => $params['req']['value']]);
    });
    $response = [];
    $response['req'] = $req;
    $response['success'] = true;
  ...
  public function cancel(Request $request)
  {
    ...
    $params['req'] = $req;
    DB::transaction(function () use ($params) {
      DB::table('addresses')->where('id', $params['req']['id'])->update(['cancelled' => $params['req']['value']]);
      DB::table('records')->where([['record_id', '=', $params['req']['id']],['name', '=', 'addresses']])->update(['cancelled' => $params['req']['value']]);
    });
    $response = [];
    $response['req'] = $req;
    $response['success'] = true;
  ...
  public function block(Request $request)
  {
    ...
    $params['req'] = $req;
    DB::transaction(function () use ($params) {
      DB::table('addresses')->where('id', $params['req']['id'])->update(['blocked' => $params['req']['value']]);
      DB::table('records')->where([['record_id', '=', $params['req']['id']],['name', '=', 'addresses']])->update(['blocked' => $params['req']['value']]);
    });
    $response = [];
    $response['req'] = $req;
    $response['success'] = true;
