...
class SaleInvoice extends Model
{
  ...
  {
    return $this->belongsToMany('App\Service')->withPivot(["name","amount"]);
  }
 /***
        ...
        if(isset($field['settings']['validations'])) {
          foreach($field['settings']['validations'] as $key =>$validation){
            ...
            if($key=='required'){
             ...
             if($action=='update') $concat .=$key.':sale_invoices,'.$field['model'].','.$id;
             else if ($action=='store') $concat .=$key.':sale_invoices';
            }else{
