...
class Purchase extends Model
{
  ...
  public function outgoing_checks()
  {
    return $this->belongsToMany('App\OutgoingCheck')->withPivot(["check_search_field","amount"]);
  }
  public function projects()
  {
    return $this->belongsToMany('App\Project')->withPivot(['percentage', 'amount']);
  }
  public function expenses_accounts()
  {
    return $this->belongsToMany('App\ExpensesAccount')->withPivot(['percentage', 'amount']);
  }
 /***
        ...
        if(isset($field['settings']['validations'])) {
          foreach($field['settings']['validations'] as $key =>$validation){
            ...
            if($key=='required'){
             ...
             if($action=='update') $concat .=$key.':purchases,'.$field['model'].','.$id;
             else if ($action=='store') $concat .=$key.':purchases';
            }else{
     ...
     case 'date':
      ...
      break;
        ...
        case 'month':
          ...
          if(isset($field['settings']['validations'])) {
            foreach($field['settings']['validations'] as $key =>$validation){
              if($i==0){ $concat="date|date_format:Y-m|"; $i++;}  else $concat .="|"; 
              #verifica si es un require el cual es el unico booleano que esta en el settings
              if($key=='required'){
                if($validation==true) $concat .=$key;
                else $concat .="nullable";
