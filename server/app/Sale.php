...
class Sale extends Model
{
  ...
  {
    return $this->belongsToMany('App\Product')->withPivot(["quantity","product_detail","pesos_price","dollars_price","pesos_total","dollars_total"]);
  }
  public function incoming_checks()
  {
    return $this->belongsToMany('App\IncomingCheck')->withPivot(["check_search_field","amount"]);
  }
 /***
        ...
        if(isset($field['settings']['validations'])) {
          foreach($field['settings']['validations'] as $key =>$validation){
            ...
            if($key=='required'){
             ...
             if($action=='update') $concat .=$key.':sales,'.$field['model'].','.$id;
             else if ($action=='store') $concat .=$key.':sales';
            }else{
